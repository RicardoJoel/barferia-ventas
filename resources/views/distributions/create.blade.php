...
<div class="fila">
	<div class="columna columna-1">
		<div class="title2">
			<h6>Procesos > Distribución > Nueva ></h6>
		</div>
	</div>
</div>
<form method="POST" action="{{ route('distributions.store') }}" role="form" id="frm-distribution">
	@csrf
	<div class="fila">
        <div class="columna columna-1">
					...
					<div class="fila">
						...
						<div class="columna columna-5a">
							...
							<select name="origin_id" id="origin_id" required>
								...
								<option value="{{ $index }}" {{ old('origin_id') == $index ? 'selected' : '' }}>{{ $center }}</option>
								@endforeach
							</select>
							@else
							<p>Origen</p>
							<input type="text" value="{{ Auth::user()->center->name ?? '' }}" disabled>
							<input type="hidden" name="origin_id" id="origin_id" value="{{ old('origin_id',Auth::user()->center_id) }}">
							@endif
						</div>
						<div class="columna columna-5h">
							...
							<select name="destiny_id" id="destiny_id" required>
								...
								<option value="{{ $index }}" {{ old('destiny_id') == $index ? 'selected' : '' }}>{{ $center }}</option>
								@endforeach
							</select>
						</div>
					</div>
