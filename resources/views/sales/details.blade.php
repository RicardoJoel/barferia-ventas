	...
	<div class="columna columna-1">
		<div class="form-section">
			...
			<div id="div-det">
				...
				<form method="POST" action="{{ action('SaleDetailController@store') }}" role="form" id="det_form">
					@csrf
					...
					<div class="fila">
						<div class="columna columna-3">
							...
							<select name="det_product_code" id="det_product_code" required>
								<option selected disabled hidden value="">Selecciona un producto</option>
								@foreach ($products->getWithPromos() as $index => $product)
								<option value="{{ $index }}" {{ old('det_product_code') == $index ? 'selected' : '' }}>{{ $product }}</option>
								@endforeach
							</select>
						</div>
				...
				<table id="tbl-details" class="tablealumno tbl-details" style="margin-bottom:10px">
					<thead>
						...
						<th width="20%">Precio (PEN)</th>
						<th width="20%">Cantidad</th>
						<th width="20%">Subtotal (PEN)</th>
